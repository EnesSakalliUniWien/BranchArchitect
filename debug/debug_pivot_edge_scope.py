#!/usr/bin/env python3
"""Debug script to check pivot edge scope"""
...
def main():
    ...
    for pivot_edge, subtree_solutions in solution.items():
        ...
        if problem_split in expand_splits:
            ...
            print(f"Collapse splits in pivot edge: {len(collapse_splits)}")
            print(f"Expand splits in pivot edge: {len(expand_splits)}")
            ...
            incompatible_outside_pivot = [
                s for s in incompatible_collapse if s not in collapse_splits
            ]

            print(f"\nIncompatible splits IN pivot edge: {len(incompatible_in_pivot)}")
            for s in incompatible_in_pivot:
                print(f"  - {list(s.indices)}")

            print(
                f"\nIncompatible splits OUTSIDE pivot edge: {len(incompatible_outside_pivot)}"
            )
            for s in incompatible_outside_pivot:
                print(f"  - {list(s.indices)}")

            if incompatible_outside_pivot:
                print(
                    f"\nBUG: Some incompatible splits are OUTSIDE the pivot edge scope!"
                )
                print(
                    f"These splits won't be collapsed by the pivot edge's collapse phase."
                )
