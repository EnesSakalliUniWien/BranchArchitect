...
from brancharchitect.consensus import create_majority_consensus_tree, create_majority_consensus_tree_extended, create_consensus_tree, compatible, collect_splits
...
def test_create_majority_consensus_tree_extended():
    ...
    trees = parse_newick(s)    
    tree = create_majority_consensus_tree_extended(trees)
        
    assert expected_number_of_splits == observed_number_of_splits
    assert '(D,E,((B,C),A),F)Root;' == tree.to_newick(lengths=False)


def test_create_majority_consensus_tree_extended():
    tree = create_majority_consensus_tree_extended(parse_newick(
        '((A,(B,C)),(D,(E,F)));' + 
        '((A,(B,C)),(D,(E,F)));' +
        '(((A,B),C),((D,E),F));' +        
        '((A,(B,C)),(D,(E,F)));' +
...
def test_create_majority_consensus():
    ...
    tree = create_majority_consensus_tree(parse_newick(
        '((((A:1,B:1),C:1),D:1):1);'+
        '(((A:1,B:1,C:1),D:1):1);'+
        '(((A:1,B:1,D:1),C:1):1);'
    ))
