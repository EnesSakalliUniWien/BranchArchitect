...
try:
    ...
    for i, tree in enumerate(trees):
        ...
        def count_nodes_and_metadata(node):
            """Count all nodes and those with metadata in the tree."""
            total = 1
            with_metadata = 1 if (hasattr(node, "values") and node.values) else 0

            for child in node.children:
                child_total, child_with_meta = count_nodes_and_metadata(child)
                total += child_total
                with_metadata += child_with_meta
        ...
        def show_sample_metadata(node):
            """Show sample metadata from the first node that has it."""
            if hasattr(node, "values") and node.values:
                print(f"    Sample metadata: {node.values}")
                return True
            for child in node.children:
                if show_sample_metadata(child):
                    return True
            return False
