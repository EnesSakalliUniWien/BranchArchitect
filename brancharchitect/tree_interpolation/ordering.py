"""
Tree ordering and reordering functions for interpolation.

This module handles the progressive ordering of consensus trees
and computation of lattice edge depths.
"""
...
def compute_lattice_edge_depths(
    ...
    for edge in lattice_edges:
        ...
        logger.debug(
            f"Edge {edge}: size={partition_size}, superset_of={superset_of_count} others, "
            f"tree_depth={tree_depth}, final_depth={final_depth}"
        )

    if missing_edges:
        logger.debug(
            f"Found {len(missing_edges)} edges not in tree or without depth, assigned depth 0"
        )
    logger.info(
        f"Computed depths for {len(lattice_edges)} lattice edges with subset-based ordering"
    )
