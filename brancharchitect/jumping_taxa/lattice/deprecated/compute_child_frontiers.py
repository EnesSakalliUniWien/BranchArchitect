...
def compute_child_frontiers(
    ...
    for child in parent.children:
        if child.split_indices in shared_splits:
            ...
            is_covered_by_unique = any(
                child.split_indices.bitmask & unique_split.bitmask
                == child.split_indices.bitmask
                for unique_split in children_to_process
            )

            if not is_covered_by_unique:
