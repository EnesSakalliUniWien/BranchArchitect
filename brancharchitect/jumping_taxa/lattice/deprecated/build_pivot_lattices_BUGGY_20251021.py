...
def build_conflict_matrix(
    ...
    if nesting_solutions and conflicting_cover_pairs:
        ...
        # For conflict matrix, compute total taxa count using consistent metric
        # FIX #1: Use _compute_conflict_taxa_size() for consistent taxa counting
    ...
    # Only nesting solutions exist - return solution from smallest row
        ...
        # FIX #2: Remove unreachable dead code (lines 401-408 deleted)
        # Note: No conflict pairs exist in this branch (handled in first if-block above)
