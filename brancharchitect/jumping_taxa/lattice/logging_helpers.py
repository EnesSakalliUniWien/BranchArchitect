...
# Inline solution_size to avoid circular import with matrices module
def _solution_size(sol: PartitionSet[Partition]) -> int:
    """Calculate total taxa count across all partitions in a solution."""
...
def log_nesting_only_solution(
    ...
    jt_logger.info(
        ...
        f"with solution size {_solution_size(smallest_nesting)}: {smallest_nesting}"
    )
