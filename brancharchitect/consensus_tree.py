...
def incorporate_split_counts(
    split_list: List[Tuple[int]], number_of_splits: Dict[Tuple[int], Dict[str, int]]
):
    for sp in filter(lambda s: len(s) > 1, split_list):
        number_of_splits.setdefault(sp, {"count": 0})["count"] += 1


def collect_count_of_splits(trees: List[Node]) -> Dict[tuple[int], Dict]:
    count_of_splits = {}
    for tree in trees:
...
def sort_splits_by_occurrence(
    splits: Dict[Tuple[int], Dict[str, float]]
    ...
    sorted_splits_with_counts = sorted(
        splits.items(), key=lambda item: item[1]["occurrence"], reverse=True
    )
...
def filter_by_occurrence(
    splits: Dict[Tuple[int], Dict[str, float]], eq: float
) -> List[Tuple[int]]:
...
def check_split_memory_compatibility(
    split_1: Tuple[int], tree_order: List[int], memory: List[int]
):
    if memory:
...
def check_split_compatibility(
    split_1: List[int], split_2: List[int], tree_order: List[int]
):
    x1 = set(split_1)
...
def filter_incompatible_splits(
    sorted_splits_by_occurrence: List[Tuple[int]], tree_order: List[int]
):
...
def apply_split_in_tree(split: Tuple[int], node: Node) -> Node:
    ...
    if (
        split_set.issubset(set(node.split_indices))
        and split_set != set(node.split_indices)
        or node.split_indices.issubset(set(split_set))
        and split_set != set(node.split_indices)
    ):
