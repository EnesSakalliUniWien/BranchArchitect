...
from brancharchitect.plot.paper_plot.paper_plot_constants import (
    ...
    DEFAULT_ENCLOSE_STROKE_WIDTH, DEFAULT_ENCLOSE_DASHARRAY,
    FONT_SANS_SERIF
)
...
def render_enclosures(
    svg_group: ET.Element, layout: Dict, enclose_options: Dict, colors: Dict, leaf_label_offset: float = 0.0
) -> None:
    """
    ...
    create_container_definitions(svg_group)
    ...
    for subtree_id, style in enclose_options.items():
        ...
        boundaries = calculate_enclosure_boundaries(
            all_node_coords, leaf_coords, label_y, style, colors, leaf_label_offset=leaf_label_offset
        )
        
        # Adjust boundaries for parent connection if needed
        boundaries = adjust_for_parent_connection(
            boundaries, subtree_root, node_coords
        )
        ...
        container_group = create_container_group(svg_group, subtree_id)
...
def get_node_coordinates(
    ...
    all_node_coords = [node_coords[node] for node in subtree_nodes if node in node_coords]
...
def calculate_enclosure_boundaries(
    ...
    boundaries = calculate_padding(
        min_x, min_y, max_x, max_y, style, colors
    )
...
def calculate_padding(
    ...
    Args:
        min_x, min_y: Minimum coordinates of the enclosure
        max_x, max_y: Maximum coordinates of the enclosure
        style: Style options for the enclosure
        colors: Color palette
    
    Returns:
        Dictionary containing padded boundaries (x, y, width, height)
    """
    # Add padding (MD3 uses consistent spacing)
    padding = float(style.get("padding", DEFAULT_ENCLOSE_PADDING))
    
    # Get node marker size and stroke width to ensure visual elements aren't cut off
    node_marker_size = float(colors.get("node_marker_size", 4.5))
    highlight_stroke_width = float(colors.get("highlight_stroke_width", 2.5))
    
    # Apply padding based on visual elements
    visual_element_padding = max(node_marker_size * 2, highlight_stroke_width * 2)
    effective_padding = max(padding, visual_element_padding)
    ...
    height = max(0, max_y - y + effective_padding)
...
def adjust_for_parent_connection(
    ...
    if subtree_root.parent and subtree_root.parent in node_coords:
        ...
        if (parent_coords["x"] < x or parent_coords["x"] > x + width or 
            parent_coords["y"] < y or parent_coords["y"] > y + height):
...
def create_container_group(svg_group: ET.Element, subtree_id: str) -> ET.Element:
    ...
    """
    return ET.SubElement(svg_group, "g", {
        "class": "md3-container",
...
def render_container_rectangle(
    ...
    # Check if this is a filled container or outline
    is_filled = style.get("fill", "none") != "none"
    
    # Get container colors - for print we use more subtle colors
    container_style = get_container_style(style, colors, is_filled, corner_radius)
    
    # For filled containers in print, use subtle pattern fill
    if is_filled:
    ...
    ET.SubElement(container_group, "rect", {
        ...
        "width": f"{width:.2f}",
        "height": f"{height:.2f}",
        "rx": container_style["rx"],
        "stroke": container_style["stroke"],
        "stroke-width": container_style["stroke_width"],
        "stroke-dasharray": container_style["stroke_dasharray"],
        "fill": container_style["fill"],
        "fill-opacity": container_style.get("fill_opacity", "1.0"),
        "opacity": container_style["opacity"]
    })
...
def get_container_style(
    ...
    """
    Get the style attributes for a container.
    
    Args:
        style: Style options for the enclosure
        colors: Color palette
        is_filled: Whether the container is filled
        corner_radius: Corner radius for the container
    
    Returns:
        Dictionary of style attributes
    """
    stroke_color = style.get("stroke", colors["highlight_enclose"])
    ...
    fill_opacity = style.get("fill-opacity", "0.15") 
...
def add_container_label(
    ...
    # Create text element
    text_el = ET.SubElement(container_group, "text", {
...
def calculate_label_style(style: Dict, colors: Dict) -> Dict:
    ...
    text_color = style.get("text_color", style.get("stroke", colors["highlight_enclose"]))
    opacity = style.get("opacity", "1.0")
    
    return {
        "font_family": colors.get("font_family", FONT_SANS_SERIF),
        "font_size": font_size,
        "font_weight": "500",  # MD3 uses medium weight for container labels
