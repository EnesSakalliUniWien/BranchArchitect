        ...
        if path.exists():
            try:
                ...
                if fallback_message:
                    display(
                        HTML(
                            f'<div style="color:red">Error displaying image: {e}</div>'
                        )
                    )
                return
    ...
    if display_method == "iframe" or display_method == "auto":
        try:
            ...
            if display_method == "iframe":
                if fallback_message:
                    display(
                        HTML(f'<div style="color:red">Error displaying PDF: {e}</div>')
                    )
                return
...
def display_plot_output(
    ...
    if ext in [".png", ".jpg", ".jpeg", ".svg"]:
        ...
        display(HTML(f'<div style="color:orange">Unsupported file type: {ext}</div>'))
