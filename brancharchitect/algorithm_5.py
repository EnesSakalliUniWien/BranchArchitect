...
def algorithm_6(tree_list, sorted_nodes: List, file_name=None, newick_list=[]):
    it1, it2, sorted_nodes, newick_list = delete_tree_leaves_for_coloring_newick(
        newick_list=newick_list)
    t1 = traverse(it1)
    t2 = traverse(it2)
    ...
    while True:
        ...
        for s_edge in all_sedges:
            # execute algorithm
            jumping_taxas = algorithm_6_for_sedge(s_edge, t1, t2, sorted_nodes)
...
def algo6_case_full_full(sedge: Node, t1: FunctionalTree, t2):
    return algo6(sedge, t1, t2)

def algorithm_6_for_sedge(sedge, t1, t2, sorted_nodes):

    if is_full_s_edge(t1, sedge) and is_full_s_edge(t2, sedge):
        return algo6_case_full_full(sedge, t1, t2)

    if is_full_s_edge(t1, sedge) and is_partial_s_edge(t2, sedge):
        return algo6_case_full_full(sedge, t1, t2)

    if is_partial_s_edge(t1, sedge) and is_full_s_edge(t2, sedge):
        return algo6_case_full_full(sedge, t2, t1)
...
def algo6(sedge: Node, t1: FunctionalTree, t2: FunctionalTree) -> list[Component]:
    ...
    voting_map: list[ComponentSet] = intersections
    m: list[Component] = argmax(voting_map, lambda x: count(voting_map, x))
    r: list[Component] = argmin(m, size)
    rr: list[Component] = reduce(union, r)  # type: ignore
