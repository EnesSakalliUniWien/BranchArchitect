from brancharchitect.tree import Node, SplitIndices, Splits
...
def relative_robinson_foulds_distance(tree1: Node, tree2: Node) -> float:
    splits1, splits2 = tree1.to_split(), tree2.to_split()
    set1 = set(splits1)
    set2 = set(splits2)
...
def weighted_robinson_foulds_distance(tree1: Node, tree2: Node) -> float:
    ...
    weighted_distance = sum(abs(splits1.get(split, 0) - splits2.get(split, 0)) for split in all_splits)

    return weighted_distance


def calculate_along_trajectory(trajectory: List[Node], distance_function: Callable[[Node, Node], float]) -> List[float]:
